...
class _MapScreenState extends State<MapScreen> {
  ...
  double? _cameraZoom;
  static const double _navigationZoom = 20.5;
  ...
  Future<void> _getCurrentLocation() async {
    ...
    _positionSub = Geolocator.getPositionStream(
      ...
      final snappedNow = await _snapToRoad(rawNow);
      if (!mounted) return;

      final nextLocation =
          _isNavigating ? (snappedNow ?? _currentLocation) : (snappedNow ?? rawNow);
  ...
  void _startNavigation() {
    ...
    if (_rawLocation != null) {
      _snapToRoad(_rawLocation!, force: true).then((snapped) {
        if (!mounted || snapped == null) return;
        setState(() => _currentLocation = snapped);
        if (_followUser) {
          _applyFollowCamera(zoomOverride: _navigationZoom);
        }
        _updateNavigationProgress();
      });
  ...
  void _trimRoutePolyline() {
    if (_routePolyline.length < 2 || _currentLocation == null) return;

    final start = _lastTrimIdx.clamp(0, _routePolyline.length - 1);
    ...
    // trim only if close enough AND moving forward
    if (bestDist <= _trimDistanceThreshold && bestIdx > _lastTrimIdx + 2) {
      setState(() {
        _routePolyline = _routePolyline.sublist(bestIdx - 1);
      });

      // After cutting, the index resets because list got shorter
      _lastTrimIdx = 0;
    } else {
  ...
  void _applyFollowCamera({double? zoomOverride}) {
    if (_currentLocation == null) return;
  ...
  Future<LatLng?> _snapToRoad(LatLng raw, {bool force = false}) async {
    ...
    // More conservative snapping while navigating (reduces teleport/jumps)
    final minInterval = _isNavigating ? 6 : 3;

    if (!force &&
        _lastSnapAt != null &&
        now.difference(_lastSnapAt!) < Duration(seconds: minInterval)) {
      return null;
    }
  ...
  double _routeBearingOrHeading() {
    if (_currentLocation == null) return _currentHeading;

    // Prefer polyline-based bearing for smooth curves.
    if (_routePolyline.length >= 2) {
      final polyBearing = _bearingAlongPolyline(_currentLocation!);
      if (polyBearing != null) return polyBearing;
    }
  ...
  double? _bearingAlongPolyline(LatLng current) {
    ...
    final start = _lastTrimIdx.clamp(0, _routePolyline.length - 1);
    var end = math.min(start + _trimSearchWindow, _routePolyline.length);
    if (end - start < 2) {
      end = math.min(_trimSearchWindow, _routePolyline.length);
    }
    ...
    // Look ahead along the route for a smoother curve direction.
    const double lookAheadMeters = 20;
    double acc = 0;
    int j = bestIdx;
  ...
  Widget build(BuildContext context) {
    ...
    final markers = <gmaps.Marker>{
      ...
      // Use snapped location while navigating to keep arrow on the road.
      if (_isNavigating && _currentLocation != null)
        gmaps.Marker(

...
class _MapScreenState extends State<MapScreen> {
    ...
    _weatherRefreshTimer = Timer.periodic(_weatherRefreshInterval, (_) {
      ...
      if (_mapTypeSelection == 'weather' || _avoidFloodedAreas) {
        _loadWeatherMarkers(force: true).then((_) {
          _maybeApplyFloodWeatherRerouteBalanced(force: true);
        });
      }
    });
  }
  ...
  Future<void> _fetchRoute() async {
    ...
    if (_avoidFloodedAreas) {
      ...
      if (routeHasFloodHits && routeHasRain) {
        ...
        for (final c in candidates) {
          ...
          if (_isBetterFloodRouteCandidate(
            candidate: c,
            candidateHits: hits,
            currentBest: bestCandidate,
            currentBestHits: bestHits,
          )) {
            bestHits = hits;
            bestCandidate = c;
          }
        ...
        if (bestHits >= baseHits) {
          final detour = await _buildDetourAvoidanceRouteBalanced(
        ...
        if (_isAcceptableFloodReroute(
          candidate: bestCandidate,
          candidateHits: bestHits,
          currentRoute: selectedResult,
          currentHits: baseHits,
        )) {
    ...
    await _maybeApplyFloodWeatherRerouteBalanced();
  }
...
Future<void> _maybeApplyFloodWeatherRerouteBalanced({bool force = false}) async {
  ...
  try {
    ...
    if (!_isRainingNearRoute(_routePolyline)) return;

    _rerouteInProgress = true;
    await Future.delayed(const Duration(milliseconds: 500));
    if (!mounted) return;

    final currentRoute = RouteFetchResult(
      polyline: List<LatLng>.from(_routePolyline),
      steps: List<RouteStep>.from(_routeSteps),
      legs: null,
      distanceMeters: _routePolylineDistanceMeters(_routePolyline),
      durationSeconds: _estimateRouteDurationSeconds(
    ...
    for (final candidate in candidates) {
      if (candidate.polyline.length < 5) continue;

      final hits = _countFloodHits(candidate.polyline);
      if (best == null ||
          _isBetterFloodRouteCandidate(
            candidate: candidate,
            candidateHits: hits,
            currentBest: best!,
            currentBestHits: bestFloodHits,
          )) {
        best = candidate;
        bestFloodHits = hits;
      }
    ...
    if (!_isAcceptableFloodReroute(
      candidate: best,
      candidateHits: bestFloodHits,
      currentRoute: currentRoute,
      currentHits: currentFloodHits,
    )) {
      _showRouteUpdateBottomBanner(
        'Current route remains the better balance for distance and flood risk.',
      );
      return;
    }
    ...
    _fitRouteBounds();
    _showRouteUpdateBottomBanner('Rerouted to least flooded route');
  } finally {
    _rerouteInProgress = false;
    if (mounted) {
      setState(() => _isFloodRerouteCalculating = false);
...
bool _isRainCondition(String condition) {
  ...
  bool _isBetterFloodRouteCandidate({
    required RouteFetchResult candidate,
    required int candidateHits,
    required RouteFetchResult currentBest,
    required int currentBestHits,
  }) {
    if (candidateHits != currentBestHits) {
      return candidateHits < currentBestHits;
    }

    const durationToleranceSeconds = 15.0;
    final durationDelta =
        candidate.durationSeconds - currentBest.durationSeconds;
  ...
  bool _isAcceptableFloodReroute({
    required RouteFetchResult candidate,
    required int candidateHits,
    required RouteFetchResult currentRoute,
    required int currentHits,
  }) {
    if (candidateHits >= currentHits) return false;

    final clearsAllFloodHits = candidateHits == 0;
    final maxDurationFactor = clearsAllFloodHits ? 1.6 : 1.3;
    final maxDistanceFactor = clearsAllFloodHits ? 1.6 : 1.35;
    final maxDurationDeltaSeconds = clearsAllFloodHits ? 360.0 : 180.0;
    final maxDistanceDeltaMeters = clearsAllFloodHits ? 2500.0 : 1200.0;

    final durationIncrease =
        candidate.durationSeconds - currentRoute.durationSeconds;
    final distanceIncrease =
        candidate.distanceMeters - currentRoute.distanceMeters;

    final tooSlow =
        candidate.durationSeconds >
            currentRoute.durationSeconds * maxDurationFactor &&
        durationIncrease > maxDurationDeltaSeconds;
    final tooFar =
        candidate.distanceMeters >
            currentRoute.distanceMeters * maxDistanceFactor &&
        distanceIncrease > maxDistanceDeltaMeters;

    return !(tooSlow || tooFar);
  }

  double _routePolylineDistanceMeters(List<LatLng> polyline) {
  ...
  double _estimateRouteDurationSeconds({
    required List<LatLng> polyline,
    List<RouteStep> steps = const [],
  }) {
    final stepDistanceMeters = steps.fold<double>(
      0,
      (sum, step) => sum + step.distanceMeters,
    );
    final distanceMeters = stepDistanceMeters > 0
        ? stepDistanceMeters
        : _routePolylineDistanceMeters(polyline);

    // Conservative fallback when OSRM duration is unavailable.
    return distanceMeters / (30 * 1000 / 3600);
  }
...
Future<RouteFetchResult?> _buildDetourAvoidanceRouteBalanced({
  required List<LatLng> baseRoute,
  required int currentHits,
}) async {
  if (_currentLocation == null || _destinations.isEmpty || currentHits <= 0) {
  ...
  final detourWaypoints =
      _buildSmartDetourWaypointsBalanced(hotspot, baseRoute);
  if (detourWaypoints.isEmpty) return null;
 ...
 List<LatLng> _buildSmartDetourWaypointsBalanced(
  LatLng floodCenter,
  ...
  if (floodZoneStartIdx == -1 || floodZoneEndIdx == -1) {
    return _buildFarDetourSeedsBalanced(floodCenter);
  }
  ...
  const detourDistances = <double>[180, 260, 340, 450, 600];
  final waypoints = <LatLng>[];

  for (final distance in detourDistances) {
    waypoints.add(_offsetByBearing(floodCenter, leftBearing, distance));
  }
  for (final distance in detourDistances) {
    waypoints.add(_offsetByBearing(floodCenter, rightBearing, distance));
  }

  return waypoints;
}

List<LatLng> _buildFarDetourSeedsBalanced(LatLng center) {
  const offsets = <Offset>[
    Offset(220, 0),
    Offset(-220, 0),
    Offset(0, 220),
